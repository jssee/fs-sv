...
// Derived on Vercel (BETTER_AUTH_URL/CORS_ORIGIN fall back to VERCEL_URL) or
// environment-specific, so never push the local values.
...
for (const [key, value] of env.entries()) {
	const result = spawnSync(
		...
		],
		{
			...
			encoding: "utf8",
			// Windows resolves bunx via a .cmd shim, which needs a shell.
